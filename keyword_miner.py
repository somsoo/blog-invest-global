...
def get_golden_keyword_and_source():
    ...
    if not articles:
        articles = [{"title": "Equity Markets & Portfolio Allocation Major Industry Update", "description": "Key trends, metrics, and structural changes observed in the sector.", "date": "2026"}]

    headlines = [a["title"] for a in articles]
    prompt = f"""
    Review the following latest headlines in Equity Markets & Portfolio Allocation:
    ...
    try:
        ...
        data = json.loads(res)
        kw = data.get("golden_keyword", "Equity Markets & Portfolio Allocation Trends")
    except Exception as e:
        kw = "Equity Markets & Portfolio Allocation Strategic Analysis"
